...
# Load the image
image = Image.open("C:/Users/octadion/PycharmProjects/handwritten-layoutlmv3/dataset/data/upload/1.jpg")
image.show()
test_dict, width_scale, height_scale = dataSetFormat(image)

print("test_dict['bboxes'] :", test_dict['bboxes'])
print("test_dict['tokens'] :", test_dict['tokens'])
...
model.load_state_dict(torch.load("C:/Users/octadion/PycharmProjects/handwritten-layoutlmv3/model.bin"))
...
true_prob = torch.tensor(
    np.array([output_prob.item() for idx, output_prob in enumerate(output_prob) if not is_subword[idx]]))
...
three_class = concat_torch[
    torch.where((concat_torch[:, 4] == 3) & (concat_torch[:, 3] == 0) & (concat_torch[:, 2] == 0))]
four_class = concat_torch[
    torch.where((concat_torch[:, 4] == 4) & (concat_torch[:, 3] == 0) & (concat_torch[:, 2] == 0))]
five_class = concat_torch[
    torch.where((concat_torch[:, 4] == 5) & (concat_torch[:, 3] == 0) & (concat_torch[:, 2] == 0))]

final = torch.row_stack((one_class, two_class, three_class, four_class, five_class))
unique_ = torch.unique(final, dim=0)

plot_img(test_dict['img_path'], unique_[:, :4], unique_[:, 4].tolist(), unique_[:, 5].tolist(), width_scale,
         height_scale)

# print(unique_)

first_string = test_dict['tokens'][0]

question_number = first_string[0]

question_text = first_string[1:].strip()

answer_text = ' '.join(test_dict['tokens'][1:])
